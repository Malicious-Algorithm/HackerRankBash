...
#There shouldn't be leading or trailing spaces. Please note that the uniq -c command by itself will generate the output in a different format.
...
var=$(<sampleUniq.txt)
echo "$var" | tr [:upper:] [:lower:] | uniq -ci | sed 's/^[[:blank:]]\+//g'
